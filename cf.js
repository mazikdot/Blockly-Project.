 ...
 */
 compilerflasher = function (loadFiles) {
    ...
    this.chromeUpdateForProgrammersMessage = addDismissButton('Due to a Chrome bug, to use programmers (or devices that internally use a programmer) you need to update your Chrome to version ' + this.minimumChromeVersion + ' or above.');
            ...
            if (location.indexOf('codebender.cc') != -1 &&
                ...
                // codebender plugin initialization
                    ...
                    if (!this.plugin_found) {
                      ...
                      if (check) {
                        alert = 'Firefox support is discontinued on this version. Please use Google Chrome/Chromium.';
                      }
    ...
    this.getFlashFailMessage = function (error) {
        ...
        if (window.flashing_errors[error]) {
            msg = addDismissButton(window.flashing_errors[error] + " <a href='http://feedback.codebender.cc/knowledgebase/articles/183395-usb-flashing-known-errors' target='_blank'>More Info</a>.");
        }
        else {
    ...
    function addDismissButton(message) {
        return message + ' <a class="dismiss-message-button" href="javascript:compilerflasher.setOperationOutput(\'\')">Dismiss</a>';
    }
};
...
{
    "-1": "Couldnโ€t find an Arduino on the selected port. If you are using Leonardo check that you have the correct port selected. If it is correct, try pressing the boardโ€s reset button after initiating the upload.",
